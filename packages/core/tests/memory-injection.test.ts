...
describe('AgentLoop memory injection lifecycle', () => {
  beforeEach(() => {
  ...
  test('skips memory injection for slash commands', async () => {
    ...
    expect((agent as any).pendingMemoryContext).toBeNull();
  });
  ...
  test('clears pendingMemoryContext when command is handled', async () => {
    ...
    (agent as any).pendingMemoryContext = 'some injected memory';
  ...
  test('preserves memory context for regular LLM messages', async () => {
    ...
    // Manually set pendingMemoryContext to simulate injection
    (agent as any).pendingMemoryContext = '## Relevant Memories\n\n### User Preferences\n- Prefers dark mode';
  ...
  test('memory context is removed from system messages on new injection', async () => {
    ...
    const contextAfterFirst = agent.getContext().getMessages();

    // Should have memory context v1
    expect((agent as any).pendingMemoryContext).toContain('Memories v1');

    // Second message - should replace old memory context
    await agent.process('second');

    // Should have memory context v2
    expect((agent as any).pendingMemoryContext).toContain('Memories v2');
    expect((agent as any).pendingMemoryContext).not.toContain('Memories v1');
  });
  ...
  test('session reset clears memory context', async () => {
    ...
    (agent as any).pendingMemoryContext = 'some memory context';
